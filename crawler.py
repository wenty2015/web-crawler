...
class Crawler:
    ...
    def loadHTML(self, url):
        try:
            ...
            html_content['out_links'] = getOutLinks(soup, self.title_terms)
    ...
    def crawlNextLevel(self, crawl_list, out_links):
        ...
        for length in xrange(1, list_length + 1):
            crawl_list_present = filter(lambda l: len(l[1]) >= length,
                                        crawl_list.items())
            crawl_list_present = map(lambda l: l[length - 1], crawl_list_present)
    ...
    def dumpURLInfo(self):
        f_url_info = open(result_dir + 'URL_INFO.txt')
        f_html_content = open(result_dir + 'HTML.txt')
        f_html_content_offset = open(result_dir + 'HTML_OFFSET.txt')
        for url_id, url_info in self.url_nodes.iteritems():
            # f_url_info
            domain_id = url_info['domain_id']
            basic_info = [url_id, domain_id,
                            self.domain_nodes[domain_id]['domain'],
                            url_info['url']]
...
def getOutLinks(soup, title_terms = None):
    out_links = []
    for link in soup.find_all('a'):
        link_content = [link.get('href'), link.get('title')]
        href = link_content[0]
        link_terms = set(re.findall(r"\w+", link_content[1].lower()))
        if href is not None and len(href) > 1 and isValieURLType(href) and href[0] != '#':
            ...
            if title_terms is not None:
                if len(title_terms & link_terms) > 0:
...
def processOutLinks(url_next_level):
    ''' url_next_level: [[out_link_url, in_link_url_id, out_link_title,
        out_link_domain_id]]'''
    unique_url, url_indices = np.unique(map(lambda l: l[0], url_next_level))
    out_links = {}
    for i, out_link in enumerate(zip(url_next_level, url_indices)):
        url, in_link_url_id, title, domain_id, tmp_url_id = out_link
...
if __name__ == '__main__':
    ...
    title = 'independence war causes american revolution'
